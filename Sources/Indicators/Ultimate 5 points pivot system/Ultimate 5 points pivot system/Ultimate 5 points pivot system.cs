...
namespace cAlgo.Indicators
{
    ...
    public class Ultimate5pointspivotsystem : Indicator
    {
        ...
        public override void Calculate(int index)
        {
            ...
            if( dailyCondition) calculateDailyPivots(index);
            if(weeklyCondition) calculateWeeklyPivots(index);
            if(monthlyCondition)  calculateMonthlyPivots(index);
        ...
        private void calculateMonthlyPivots(int index)
        {
            ...
            {
            	if(monthlyLow > MarketSeries.Low[index]) monthlyLow = MarketSeries.Low[index];
            	if(monthlyHigh < MarketSeries.High[index]) monthlyHigh = MarketSeries.High[index];
        ...
        private void calculateWeeklyPivots(int index)
        {
            ...
            {
            	if(weeklyLow > MarketSeries.Low[index]) weeklyLow = MarketSeries.Low[index];
            	if(weeklyHigh < MarketSeries.High[index]) weeklyHigh = MarketSeries.High[index];
        ...
        protected void calculateDailyPivots(int index)
        {
            ...
            if(cond1)
            {
            	...
            	if(dailyLow > MarketSeries.Low[index]) dailyLow = MarketSeries.Low[index];
            	if(dailyHigh < MarketSeries.High[index]) dailyHigh = MarketSeries.High[index];
