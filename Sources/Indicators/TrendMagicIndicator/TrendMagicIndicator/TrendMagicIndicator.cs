...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class TrendMagicIndicator : Indicator
    {
        Mq4Double Mq4Init()
        {
        ...
        Mq4Double Mq4Start()
        {
            ...
            for (shift = limit; shift >= 0; shift--)
            {
                ...
                if (thisCCI >= 0 && lastCCI < 0)
                    bufferUp[shift + 1] = bufferDn[shift + 1];

                if (thisCCI <= 0 && lastCCI > 0)
                    bufferDn[shift + 1] = bufferUp[shift + 1];
        ...
        [Parameter("CCI", DefaultValue = 50)]
        public int CCI_parameter { get; set; }
        bool _CCIGot = false;
        Mq4Double CCI_backfield;
        Mq4Double CCI
        {
            get
            {
                if (!_CCIGot)
                    CCI_backfield = CCI_parameter;
                return CCI_backfield;
            }
            set { CCI_backfield = value; }
        }

        [Parameter("ATR", DefaultValue = 5)]
        public int ATR_parameter { get; set; }
        bool _ATRGot = false;
        Mq4Double ATR_backfield;
        Mq4Double ATR
        {
            get
            {
                if (!_ATRGot)
                    ATR_backfield = ATR_parameter;
                return ATR_backfield;
            }
            set { ATR_backfield = value; }
        }

        [Output("bufferUp", Color = Colors.Green, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries BufferUpOutput { get; set; }

        [Output("bufferDn", Color = Colors.Red, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries BufferDnOutput { get; set; }
        ...
        private Mq4OutputDataSeries bufferUp;
        private Mq4OutputDataSeries bufferDn;
        ...
        {
            if (BufferUpOutput == null)
                BufferUpOutput = CreateDataSeries();

            bufferUp = new Mq4OutputDataSeries(this, BufferUpOutput, ChartObjects, 0, 0, () => CreateDataSeries(), 2, Colors.Blue);
            AllBuffers.Add(bufferUp);

            if (BufferDnOutput == null)
                BufferDnOutput = CreateDataSeries();
            bufferDn = new Mq4OutputDataSeries(this, BufferDnOutput, ChartObjects, 0, 1, () => CreateDataSeries(), 2, Colors.Red);
            AllBuffers.Add(bufferDn);

            AllOutputDataSeries.Add(BufferUpOutput);
            AllOutputDataSeries.Add(BufferDnOutput);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                bufferUp.SetCurrentIndex(index);
                bufferDn.SetCurrentIndex(index);
            ...
            {
                if (e.Source != null)
                    Print("IOException source: {0}", e.Source);
                throw;
            }
        }
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly TrendMagicIndicator _indicator;

            public Mq4OutputDataSeries(TrendMagicIndicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = TrendMagicIndicator.GetArrowByCode(ArrowCode);
