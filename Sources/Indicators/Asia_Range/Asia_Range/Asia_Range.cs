...
{
    [Indicator(ScalePrecision = 5, AutoRescale = false, IsOverlay = true, TimeZone = TimeZones.EEuropeStandardTime, AccessRights = AccessRights.None)]
    [Levels()]
    public class Asia_Range : Indicator
    {
        void Mq4Init()
        {
            Mq4Double i = 0;
            ...
            {
                CreateLinesFunc("upLine" + i, indicator_color1);
                CreateLinesFunc("dnLine" + i, indicator_color2);
            }
            return;
        }
        void CreateLinesFunc(Mq4String no, Mq4Double cl)
        {
        ...
        void Mq4Start()
        {
            ...
            if (ShowHistory)
            {
                dt = decDateTradeDayFunc(CurTime());
                for (i = 0; i < NumberOfDays; i++)
                {
                    DrawLinesFunc(dt, i);
            ...
            if (ShowComment)
                Comment("CheckTime=" + CheckTime);
            return;
        }
        void SetHLineFunc(Mq4Double nl, Mq4Double pp)
        {
            Mq4Double cl = 0;
        ...
        void DrawLinesFunc(Mq4Double dt, Mq4Double nd)
        {
            Mq4Double b2 = 0;
            Mq4Double b1 = 0;
            Mq4Double p2 = 0;
        ...
        [Parameter("CheckTime", DefaultValue = "08:00")]
        public string CheckTime_parameter { get; set; }
        bool _CheckTimeGot;
        Mq4String CheckTime_backfield;
        Mq4String CheckTime
        {
            get
            {
                if (!_CheckTimeGot)
                    CheckTime_backfield = CheckTime_parameter;
                return CheckTime_backfield;
            }
            set { CheckTime_backfield = value; }
        }

        [Parameter("ShowHistory", DefaultValue = True)]
        public bool ShowHistory_parameter { get; set; }
        bool _ShowHistoryGot;
        Mq4Double ShowHistory_backfield;
        Mq4Double ShowHistory
        {
            get
            {
                if (!_ShowHistoryGot)
                    ShowHistory_backfield = ShowHistory_parameter;
                return ShowHistory_backfield;
            }
            set { ShowHistory_backfield = value; }
        }

        [Parameter("NumberOfDays", DefaultValue = 5)]
        ...
        [Parameter("ShowComment", DefaultValue = True)]
        public bool ShowComment_parameter { get; set; }
        bool _ShowCommentGot;
        Mq4Double ShowComment_backfield;
        Mq4Double ShowComment
        {
            get
            {
                if (!_ShowCommentGot)
                    ShowComment_backfield = ShowComment_parameter;
                return ShowComment_backfield;
            }
            set { ShowComment_backfield = value; }
        }



        public IndicatorDataSeries dBuf0_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries dBuf1_AlgoOutputDataSeries { get; set; }




        int indicator_buffers = 2;
        int indicator_color1 = SkyBlue;
        int indicator_color2 = DarkGreen;
        ...
        private Mq4OutputDataSeries dBuf0;
        private Mq4OutputDataSeries dBuf1;
        ...
        {
            if (dBuf0_AlgoOutputDataSeries == null)
                dBuf0_AlgoOutputDataSeries = CreateDataSeries();
            dBuf0 = new Mq4OutputDataSeries(this, dBuf0_AlgoOutputDataSeries, ChartObjects, 3, 0, () => CreateDataSeries(), 1, Colors.SkyBlue);
            AllBuffers.Add(dBuf0);
            if (dBuf1_AlgoOutputDataSeries == null)
                dBuf1_AlgoOutputDataSeries = CreateDataSeries();
            dBuf1 = new Mq4OutputDataSeries(this, dBuf1_AlgoOutputDataSeries, ChartObjects, 3, 1, () => CreateDataSeries(), 1, Colors.DarkGreen);
            AllBuffers.Add(dBuf1);

            AllOutputDataSeries.Add(dBuf0_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(dBuf1_AlgoOutputDataSeries);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                dBuf0.SetCurrentIndex(index);
                dBuf1.SetCurrentIndex(index);
...
//{
        ...
        class Mq4OutputDataSeries : IMq4Array<Mq4Double>
        {
            ...
            private readonly Asia_Range _indicator;

            public Mq4OutputDataSeries(Asia_Range indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = Asia_Range.GetArrowByCode(ArrowCode);
