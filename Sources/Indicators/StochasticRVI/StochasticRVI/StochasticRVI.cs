...
namespace cAlgo.Indicators
{
    ...
    class StochasticRVI : Indicator
    {
        ...
        public override void Calculate(int index)
        {
            ...
            _buffer1[index] = ((MarketSeries.Close[index] - MarketSeries.Open[index])
                + 2 * (MarketSeries.Close[index - 1] - MarketSeries.Open[index - 1])
                + 2 * (MarketSeries.Close[index - 2] - MarketSeries.Open[index - 2])
                + (MarketSeries.Close[index - 3] - MarketSeries.Open[index - 3])) / 6;

            _buffer2[index] = ((MarketSeries.High[index] - MarketSeries.Low[index])
                + 2 * (MarketSeries.High[index - 1] - MarketSeries.Low[index - 1])
                + 2 * (MarketSeries.High[index - 2] - MarketSeries.Low[index - 2])
                + (MarketSeries.High[index - 3] - MarketSeries.Low[index - 3])) / 6;
            ...
            Trigger[index] = .96*(RVI[index-1] + .02);            
