...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class MTFCandles_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
            IndicatorShortName("MTF Candles (" + TimeFrameMq4 + ")");
        ...
        Mq4Double Mq4Start()
        {
            ...
            for (i = Bars - 1 - IndicatorCounted(); i >= 0; i--)
            {
                ...
                EMA_H[i] = iMA(Sym, TimeFrameMq4, 34, TimeFrameMq4, MODE_EMA, PRICE_HIGH, i);
                EMA_L[i] = iMA(Sym, TimeFrameMq4, 34, TimeFrameMq4, MODE_EMA, PRICE_LOW, i);
                EMA_C[i] = iMA(Sym, TimeFrameMq4, 34, TimeFrameMq4, MODE_EMA, PRICE_CLOSE, i);
                shift1 = iBarShift(NULL, TimeFrameMq4, Time[i]);
                time1 = iTime(NULL, TimeFrameMq4, shift1);
                shift2 = iBarShift(NULL, 0, time1);

                high = iHigh(NULL, TimeFrameMq4, shift1);
                low = iLow(NULL, TimeFrameMq4, shift1);
                open = iOpen(NULL, TimeFrameMq4, shift1);
                close = iClose(NULL, TimeFrameMq4, shift1);
                bodyHigh = MathMax(open, close);
                bodyLow = MathMin(open, close);

                if (open <= close && close < EMA_H[i] && close > EMA_L[i])
                {
                    RangeWickUp[shift2] = high;
                    RangeCandleUp[shift2] = bodyHigh;
                    RangeWickDown[shift2] = low;
                    RangeCandleDown[shift2] = bodyLow;
                }
                else if (open >= close && close < EMA_H[i] && close > EMA_L[i])
                {
                    RangeWickUp[shift2] = low;
                    RangeCandleUp[shift2] = bodyLow;
                    RangeWickDown[shift2] = high;
                    RangeCandleDown[shift2] = bodyHigh;
                }
            }
        ...
        [Parameter("TimeFrameMq4", DefaultValue = 0)]
        ...
        [Parameter("BarWidth", DefaultValue = 1)]
        public int BarWidth_parameter { get; set; }
        bool _BarWidthGot;
        Mq4Double BarWidth_backfield;
        Mq4Double BarWidth
        {
            get
            {
                if (!_BarWidthGot)
                    BarWidth_backfield = BarWidth_parameter;
                return BarWidth_backfield;
            }
            set { BarWidth_backfield = value; }
        }

        [Parameter("CandleWidth", DefaultValue = 2)]
        public int CandleWidth_parameter { get; set; }
        bool _CandleWidthGot;
        Mq4Double CandleWidth_backfield;
        Mq4Double CandleWidth
        {
            get
            {
                if (!_CandleWidthGot)
                    CandleWidth_backfield = CandleWidth_parameter;
                return CandleWidth_backfield;
            }
            set { CandleWidth_backfield = value; }
        }



        public IndicatorDataSeries RangeWickUp_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries RangeWickDown_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries RangeCandleUp_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries RangeCandleDown_AlgoOutputDataSeries { get; set; }
        [Output("EMA_H", Color = Colors.Silver, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries EMA_H_AlgoOutputDataSeries { get; set; }
        [Output("EMA_L", Color = Colors.Silver, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries EMA_L_AlgoOutputDataSeries { get; set; }
        [Output("EMA_C", Color = Colors.Silver, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries EMA_C_AlgoOutputDataSeries { get; set; }


        Mq4String Sym = "";

        int indicator_buffers = 10;
        int indicator_color1 = LightSteelBlue;
        int indicator_color2 = SteelBlue;
        int indicator_color3 = LightSteelBlue;
        int indicator_color4 = SteelBlue;
        int indicator_color5 = Silver;
        int indicator_color6 = Silver;
        int indicator_color7 = Silver;
        ...
        private Mq4OutputDataSeries RangeWickUp;
        private Mq4OutputDataSeries RangeWickDown;
        private Mq4OutputDataSeries RangeCandleUp;
        private Mq4OutputDataSeries RangeCandleDown;
        private Mq4OutputDataSeries EMA_H;
        private Mq4OutputDataSeries EMA_L;
        private Mq4OutputDataSeries EMA_C;
        ...
        {
            if (RangeWickUp_AlgoOutputDataSeries == null)
                RangeWickUp_AlgoOutputDataSeries = CreateDataSeries();
            RangeWickUp = new Mq4OutputDataSeries(this, RangeWickUp_AlgoOutputDataSeries, ChartObjects, 2, 0, () => CreateDataSeries(), 1, Colors.LightSteelBlue);
            AllBuffers.Add(RangeWickUp);
            if (RangeWickDown_AlgoOutputDataSeries == null)
                RangeWickDown_AlgoOutputDataSeries = CreateDataSeries();
            RangeWickDown = new Mq4OutputDataSeries(this, RangeWickDown_AlgoOutputDataSeries, ChartObjects, 2, 1, () => CreateDataSeries(), 1, Colors.SteelBlue);
            AllBuffers.Add(RangeWickDown);
            if (RangeCandleUp_AlgoOutputDataSeries == null)
                RangeCandleUp_AlgoOutputDataSeries = CreateDataSeries();
            RangeCandleUp = new Mq4OutputDataSeries(this, RangeCandleUp_AlgoOutputDataSeries, ChartObjects, 2, 2, () => CreateDataSeries(), 1, Colors.LightSteelBlue);
            AllBuffers.Add(RangeCandleUp);
            if (RangeCandleDown_AlgoOutputDataSeries == null)
                RangeCandleDown_AlgoOutputDataSeries = CreateDataSeries();
            RangeCandleDown = new Mq4OutputDataSeries(this, RangeCandleDown_AlgoOutputDataSeries, ChartObjects, 2, 3, () => CreateDataSeries(), 1, Colors.SteelBlue);
            AllBuffers.Add(RangeCandleDown);
            if (EMA_H_AlgoOutputDataSeries == null)
                EMA_H_AlgoOutputDataSeries = CreateDataSeries();
            EMA_H = new Mq4OutputDataSeries(this, EMA_H_AlgoOutputDataSeries, ChartObjects, 0, 4, () => CreateDataSeries(), 1, Colors.Silver);
            AllBuffers.Add(EMA_H);
            if (EMA_L_AlgoOutputDataSeries == null)
                EMA_L_AlgoOutputDataSeries = CreateDataSeries();
            EMA_L = new Mq4OutputDataSeries(this, EMA_L_AlgoOutputDataSeries, ChartObjects, 0, 5, () => CreateDataSeries(), 1, Colors.Silver);
            AllBuffers.Add(EMA_L);
            if (EMA_C_AlgoOutputDataSeries == null)
                EMA_C_AlgoOutputDataSeries = CreateDataSeries();
            EMA_C = new Mq4OutputDataSeries(this, EMA_C_AlgoOutputDataSeries, ChartObjects, 0, 6, () => CreateDataSeries(), 1, Colors.Silver);
            AllBuffers.Add(EMA_C);

            AllOutputDataSeries.Add(RangeWickUp_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(RangeWickDown_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(RangeCandleUp_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(RangeCandleDown_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(EMA_H_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(EMA_L_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(EMA_C_AlgoOutputDataSeries);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                RangeWickUp.SetCurrentIndex(index);
                RangeWickDown.SetCurrentIndex(index);
                RangeCandleUp.SetCurrentIndex(index);
                RangeCandleDown.SetCurrentIndex(index);
                EMA_H.SetCurrentIndex(index);
                EMA_L.SetCurrentIndex(index);
                EMA_C.SetCurrentIndex(index);
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly MTFCandles_Indicator _indicator;

            public Mq4OutputDataSeries(MTFCandles_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = MTFCandles_Indicator.GetArrowByCode(ArrowCode);
