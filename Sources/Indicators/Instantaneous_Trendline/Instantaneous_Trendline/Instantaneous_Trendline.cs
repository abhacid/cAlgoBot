    ...
    public class ITrend : Indicator
    {
        ...
        public override void Calculate(int index)
        {
            ...
            else
            {
                if (index < 7)
                {
                    itrend[index] = ((price[index] + 2 * price[index - 1] + price[index - 2]) / 4);
                }
                else
                {
                    itrend[index] = (Alpha - Alpha * Alpha / 4) * price[index] + 0.5 * Alpha * Alpha * price[index - 1] - (Alpha - 0.75 * Alpha * Alpha) * price[index - 2] + 2 * (1 - Alpha) * itrend[index - 1] - (1 - Alpha) * (1 - Alpha) * itrend[index - 2];
                }
