...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class JJNFibo_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
        ...
        Mq4Double Mq4Start()
        {
            Mq4Double DisplayDec = 0;
            Mq4Double Is_JPY = 0;
            Mq4Double countbars = 0;
            Mq4Double shift_close = 0;
            Mq4Double j = 0;
            Mq4Double k = 0;
            Mq4Double summabars = 0;
            Mq4Double i = 0;
            Mq4DoubleArray unitbars = new Mq4DoubleArray();
            Mq4DoubleArray shift_open = new Mq4DoubleArray();
            Mq4DoubleArray starttime = new Mq4DoubleArray();
            ...
            for (i = 0; i < ProcessedUnits; i++)
            {
                starttime[i] = iTime(NULL, BasisTF, i);
                if (BasisTF == 10080)
                    shift_open[i] = iBarShift(NULL, 0, starttime[i]) - 1;
                else
                    shift_open[i] = iBarShift(NULL, 0, starttime[i]);

                if (i != 0)
                    unitbars[i] = iBarShift(NULL, 0, iTime(NULL, BasisTF, i)) - iBarShift(NULL, 0, iTime(NULL, BasisTF, i - 1));
            }
            unitbars[0] = iBarShift(NULL, 0, starttime[0]) + 1;

            summabars = shift_open[ProcessedUnits - 1] + 1;

            k = 1;
            for (j = ProcessedUnits - 1; j >= 0; j--)
            {
                for (i = unitbars[j] - 1; i >= 0; i--)
                {

                    shift_close = iBarShift(NULL, 0, Time[summabars - k]);
                    countbars = shift_open[j] - shift_close + 1;
            ...
            Is_JPY = StringFind(Symbol.Code, "JPY", 0);
            if (Is_JPY == -1)
            {
                DisplayDec = DisplayDecimals;
            }
            else if (Is_JPY != -1)
            {
        ...
        [Parameter("ProcessedUnits", DefaultValue = 1)]
        public int ProcessedUnits_parameter { get; set; }
        bool _ProcessedUnitsGot;
        Mq4Double ProcessedUnits_backfield;
        Mq4Double ProcessedUnits
        {
            get
            {
                if (!_ProcessedUnitsGot)
                    ProcessedUnits_backfield = ProcessedUnits_parameter;
                return ProcessedUnits_backfield;
            }
            set { ProcessedUnits_backfield = value; }
        }

        [Parameter("TextColor", DefaultValue = Black)]
        public int TextColor_parameter { get; set; }
        bool _TextColorGot;
        Mq4Double TextColor_backfield;
        Mq4Double TextColor
        {
            get
            {
                if (!_TextColorGot)
                    TextColor_backfield = TextColor_parameter;
                return TextColor_backfield;
            }
            set { TextColor_backfield = value; }
        }

        [Parameter("Help_For_BasisTF", DefaultValue = "M:43200;W:10080;D1:1440;H4:240;H1:60")]
        public string Help_For_BasisTF_parameter { get; set; }
        bool _Help_For_BasisTFGot;
        Mq4String Help_For_BasisTF_backfield;
        Mq4String Help_For_BasisTF
        {
            get
            {
                if (!_Help_For_BasisTFGot)
                    Help_For_BasisTF_backfield = Help_For_BasisTF_parameter;
                return Help_For_BasisTF_backfield;
            }
            set { Help_For_BasisTF_backfield = value; }
        }

        [Parameter("BasisTF", DefaultValue = PERIOD_D1)]
        public int BasisTF_parameter { get; set; }
        bool _BasisTFGot;
        Mq4Double BasisTF_backfield;
        Mq4Double BasisTF
        {
            get
            {
                if (!_BasisTFGot)
                    BasisTF_backfield = BasisTF_parameter;
                return BasisTF_backfield;
            }
            set { BasisTF_backfield = value; }
        }

        [Parameter("Help_for_DisplayDecimals", DefaultValue = "Used only: 4,5!")]
        public string Help_for_DisplayDecimals_parameter { get; set; }
        bool _Help_for_DisplayDecimalsGot;
        Mq4String Help_for_DisplayDecimals_backfield;
        Mq4String Help_for_DisplayDecimals
        {
            get
            {
                if (!_Help_for_DisplayDecimalsGot)
                    Help_for_DisplayDecimals_backfield = Help_for_DisplayDecimals_parameter;
                return Help_for_DisplayDecimals_backfield;
            }
            set { Help_for_DisplayDecimals_backfield = value; }
        }

        [Parameter("DisplayDecimals", DefaultValue = 4)]
        public int DisplayDecimals_parameter { get; set; }
        bool _DisplayDecimalsGot;
        Mq4Double DisplayDecimals_backfield;
        Mq4Double DisplayDecimals
        {
            get
            {
                if (!_DisplayDecimalsGot)
                    DisplayDecimals_backfield = DisplayDecimals_parameter;
                return DisplayDecimals_backfield;
            }
            set { DisplayDecimals_backfield = value; }
        }



        [Output("Fib0", Color = Colors.Chocolate, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib0_AlgoOutputDataSeries { get; set; }
        [Output("Fib1", Color = Colors.DarkSlateGray, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib1_AlgoOutputDataSeries { get; set; }
        [Output("Fib2", Color = Colors.DarkSlateGray, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib2_AlgoOutputDataSeries { get; set; }
        [Output("Fib3", Color = Colors.Chocolate, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib3_AlgoOutputDataSeries { get; set; }
        [Output("Fib4", Color = Colors.DarkSlateGray, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib4_AlgoOutputDataSeries { get; set; }
        [Output("Fib5", Color = Colors.Chocolate, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib5_AlgoOutputDataSeries { get; set; }
        [Output("Fib6", Color = Colors.DarkSlateGray, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries Fib6_AlgoOutputDataSeries { get; set; }
        ...
        int indicator_color1 = Chocolate;
        int indicator_color2 = DarkSlateGray;
        int indicator_color3 = DarkSlateGray;
        int indicator_color4 = Chocolate;
        int indicator_color5 = DarkSlateGray;
        int indicator_color6 = Chocolate;
        int indicator_color7 = DarkSlateGray;
        ...
        private Mq4OutputDataSeries Fib0;
        private Mq4OutputDataSeries Fib1;
        private Mq4OutputDataSeries Fib2;
        private Mq4OutputDataSeries Fib3;
        private Mq4OutputDataSeries Fib4;
        private Mq4OutputDataSeries Fib5;
        private Mq4OutputDataSeries Fib6;
        ...
        {
            if (Fib0_AlgoOutputDataSeries == null)
                Fib0_AlgoOutputDataSeries = CreateDataSeries();
            Fib0 = new Mq4OutputDataSeries(this, Fib0_AlgoOutputDataSeries, ChartObjects, 0, 0, () => CreateDataSeries(), 2, Colors.Chocolate);
            AllBuffers.Add(Fib0);
            if (Fib1_AlgoOutputDataSeries == null)
                Fib1_AlgoOutputDataSeries = CreateDataSeries();
            Fib1 = new Mq4OutputDataSeries(this, Fib1_AlgoOutputDataSeries, ChartObjects, 0, 1, () => CreateDataSeries(), 1, Colors.DarkSlateGray);
            AllBuffers.Add(Fib1);
            if (Fib2_AlgoOutputDataSeries == null)
                Fib2_AlgoOutputDataSeries = CreateDataSeries();
            Fib2 = new Mq4OutputDataSeries(this, Fib2_AlgoOutputDataSeries, ChartObjects, 0, 2, () => CreateDataSeries(), 1, Colors.DarkSlateGray);
            AllBuffers.Add(Fib2);
            if (Fib3_AlgoOutputDataSeries == null)
                Fib3_AlgoOutputDataSeries = CreateDataSeries();
            Fib3 = new Mq4OutputDataSeries(this, Fib3_AlgoOutputDataSeries, ChartObjects, 0, 3, () => CreateDataSeries(), 1, Colors.Chocolate);
            AllBuffers.Add(Fib3);
            if (Fib4_AlgoOutputDataSeries == null)
                Fib4_AlgoOutputDataSeries = CreateDataSeries();
            Fib4 = new Mq4OutputDataSeries(this, Fib4_AlgoOutputDataSeries, ChartObjects, 0, 4, () => CreateDataSeries(), 1, Colors.DarkSlateGray);
            AllBuffers.Add(Fib4);
            if (Fib5_AlgoOutputDataSeries == null)
                Fib5_AlgoOutputDataSeries = CreateDataSeries();
            Fib5 = new Mq4OutputDataSeries(this, Fib5_AlgoOutputDataSeries, ChartObjects, 0, 5, () => CreateDataSeries(), 2, Colors.Chocolate);
            AllBuffers.Add(Fib5);
            if (Fib6_AlgoOutputDataSeries == null)
                Fib6_AlgoOutputDataSeries = CreateDataSeries();
            Fib6 = new Mq4OutputDataSeries(this, Fib6_AlgoOutputDataSeries, ChartObjects, 0, 6, () => CreateDataSeries(), 1, Colors.DarkSlateGray);
            AllBuffers.Add(Fib6);

            AllOutputDataSeries.Add(Fib0_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Fib1_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Fib2_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Fib3_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Fib4_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Fib5_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Fib6_AlgoOutputDataSeries);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                Fib0.SetCurrentIndex(index);
                Fib1.SetCurrentIndex(index);
                Fib2.SetCurrentIndex(index);
                Fib3.SetCurrentIndex(index);
                Fib4.SetCurrentIndex(index);
                Fib5.SetCurrentIndex(index);
                Fib6.SetCurrentIndex(index);
...
//}
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly JJNFibo_Indicator _indicator;

            public Mq4OutputDataSeries(JJNFibo_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
...
//{
        ...
        class Mq4Text : Mq4Object
        {
            ...
            public Mq4Text(string name, int type, ChartObjects chartObjects, TimeSeries timeSeries) : base(name, type, chartObjects)
            {
                _timeSeries = timeSeries;
            }
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = JJNFibo_Indicator.GetArrowByCode(ArrowCode);
